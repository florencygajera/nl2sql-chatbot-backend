...
async def handle_chat(message: str, db: Session, cached_schema: str = "") -> dict[str, Any]:
    ...
    # ── Security validation with repair loop ─────────────────────────────────
    max_guard_retries = 2
    guard_attempt = 0
    current_sql = raw_sql

    while guard_attempt <= max_guard_retries:
        try:
            validation = validate_and_sanitize(current_sql, dialect=dialect)
            safe_sql = validation.sanitized_sql

            ok, reason = _basic_sql_sanity(safe_sql)
            if not ok:
                # one fast retry: tell model specifically to add FROM
                if "Missing FROM clause" in reason and guard_attempt < max_guard_retries:
                    repair_prompt = (
                        "Return ONLY a valid SQL Server SELECT query WITH a FROM clause.\n"
                        "Use ONLY schema tables/columns.\n"
                        f"QUESTION: {message}\n\nSCHEMA:\n{schema}\n"
                    )
                    raw2 = await generate_sql(repair_prompt, schema_hint=schema, dialect=dialect)
                    validation2 = validate_and_sanitize(raw2, dialect=dialect)
                    safe_sql = validation2.sanitized_sql
                    current_sql = safe_sql  # Update current_sql for potential retry

                    ok2, reason2 = _basic_sql_sanity(safe_sql)
                    if not ok2:
                        # Fast retry failed, continue to main repair loop
                        guard_attempt += 1
                        continue
                else:
                    return {"type": "CHAT", "answer": f"I blocked invalid SQL generated by the model: {reason}", "sql": safe_sql}

            break  # Success - exit the loop
        except SQLGuardError as exc:
            error_msg = str(exc)
            logger.warning("SQL guard rejected query (attempt %d): %s | SQL: %s", guard_attempt, error_msg, current_sql)

            if guard_attempt >= max_guard_retries:
                return {
                    "type": "CHAT",
                    "answer": f"I couldn't generate valid SQL after {max_guard_retries + 1} attempts. Error: {error_msg}. Please rephrase your question.",
                }
            ...
            repair_prompt = (
                ...
                "HARD RULES:\n"
                "- Must be a valid SELECT statement with FROM clause\n"
            ...
            try:
                ...
                logger.error("Failed to repair SQL: %s", repair_exc)
                return {
                    "type": "CHAT",
                    "answer": f"The generated SQL did not pass validation: {error_msg}. Please rephrase your question.",
                }
    ...
    # ── Basic sanity checks BEFORE execution (with repair) ───────────────────
    sanity_attempt = 0
    max_sanity_retries = 2
    current_sql = safe_sql

    while sanity_attempt <= max_sanity_retries:
        ok, reason = _basic_sql_sanity(current_sql)
        if ok:
            safe_sql = current_sql
            break

        logger.warning("SQL sanity check failed (attempt %d): %s | SQL: %s", sanity_attempt, reason, current_sql)

        if sanity_attempt >= max_sanity_retries:
            return {
                "type": "CHAT",
        ...
        repair_prompt = (
            "Fix this SQL query. Return ONLY corrected SQL.\n"
            "HARD RULES:\n"
            "- Must be a valid SELECT statement with proper FROM clause\n"
            "- Use ONLY tables and columns present in the schema\n"
            "- Do NOT use markdown code blocks\n"
            f"\nUSER QUESTION:\n{message}\n\n"
            f"FAILED SQL:\n{current_sql}\n\n"
